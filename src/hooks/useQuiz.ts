...
export function useQuiz(lessonId: string) {
  ...
  const generateQuizAI = useCallback(async (questionCount: number = 5) => {
    setLoading(true)
    setError(null)
    try {
      const res = await quizzesApi.generateQuizAI(lessonId, questionCount)
      setQuiz(res.data)
      if (res.data.questions) {
        setQuestions(res.data.questions)
      }
      toasts.add({ type: 'success', title: 'Quiz generated by AI successfully!' })
      return res.data
    } catch (err: any) {
      toasts.add({ type: 'error', title: 'Error', message: err.response?.data?.detail || 'Failed to generate quiz with AI' })
      throw err
    } finally {
      setLoading(false)
    }
