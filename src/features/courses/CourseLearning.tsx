            ...
            <>
                ...
                <>
                  {currentItem.kind === 'material' ? (
                    ...
                    <LessonMultiModalWorkspace
                      ...
                      onVideoEnded={() => {
                        if (!completedLessons.has(currentItem.id)) {
                          toggleLessonCompleted(currentItem.id)
                        }
                      }}
              ...
              {activeWorkspaceTab === 'notes' && (
                <Card padding="responsive">
                  <NotesPanel
                    courseId={course.id}
                    lessonId={currentItem.kind === 'lesson' ? currentItem.id : undefined}
                    lessonTitle={currentItem.kind === 'lesson' ? currentItem.title : undefined}
                  />
                </Card>
              )}
            </>
...
function NotesPanel({ courseId, lessonId, lessonTitle }: { courseId: string; lessonId?: string; lessonTitle?: string }) {
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2 pb-2 border-b border-border">
        ...
        <h3 className="font-semibold text-foreground text-sm">
          Ghi chú{lessonTitle ? ` – ${lessonTitle}` : ''}
        </h3>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-8 text-center">
          ...
          <p className="text-sm text-muted-foreground">Chưa có ghi chú nào. Hãy thêm ghi chú đầu tiên!</p>
        </div>
      ) : (
        <div className="space-y-3">
          {notes.map(note => (
