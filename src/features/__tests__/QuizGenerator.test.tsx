  ...
  useAppStore: (selector: (state: Record<string, unknown>) => unknown) => {
    const state = {
      documents: {
        ...
        selectedId: null,
        select: vi.fn(), add: vi.fn(), remove: vi.fn(), retry: vi.fn(),
        updateProgress: vi.fn(), loadDocuments: vi.fn(), uploadDocument: vi.fn(),
      },
...
describe("QuizGenerator", () => {
  it("renders quiz generator page", () => {
    ...
    expect(screen.getByText(/trắc nghiệm ai/i)).toBeInTheDocument()
    expect(screen.getByText(/Bắt đầu tạo đề trắc nghiệm/i)).toBeInTheDocument()
  })
